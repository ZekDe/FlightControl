...
int8_t ekfCorrect(EKF_Handle_t *handle, const float *accel)
{
     ...
     * 
     * Step 1: temp3x7 = H * P  (but H is stored in handle->H)
     *         Actually: temp7x3 = P * H'
     * Step 2: S = H * temp7x3 + R
     */
    
    /* temp3x7 = H (already set) */
    /* temp7x3 = P * H' */
    ret = matrixTranspose(&handle->H, &handle->temp7x3);  /* temp7x3 = H' */
    if (ret != MATRIX_OK) return EKF_ERR_MATRIX_OP;
    
    Matrix_t PH_T;  /* P * H' */
    matrixInit(&PH_T, EKF_STATE_DIM, EKF_MEAS_DIM);
    ret = matrixMul(&handle->P, &handle->temp7x3, &PH_T);
    if (ret != MATRIX_OK) return EKF_ERR_MATRIX_OP;
    
    /* S = H * P * H' */
    ret = matrixMul(&handle->H, &PH_T, &handle->temp3x3);
    if (ret != MATRIX_OK) return EKF_ERR_MATRIX_OP;
    ...
    /* Compute S^(-1) */
    Matrix_t S_inv;
    matrixInit(&S_inv, EKF_MEAS_DIM, EKF_MEAS_DIM);
    ret = matrixInverse(&handle->temp3x3, &S_inv);
     ...
     * K = PH_T * S_inv
     */
    ret = matrixMul(&PH_T, &S_inv, &handle->K);
    if (ret != MATRIX_OK) return EKF_ERR_MATRIX_OP;
    ...
    /* P_new = (I - K * H) * P */
    Matrix_t P_new;
    matrixInit(&P_new, EKF_STATE_DIM, EKF_STATE_DIM);
    ret = matrixMul(&handle->temp7x7_2, &handle->P, &P_new);
    if (ret != MATRIX_OK) return EKF_ERR_MATRIX_OP;
    
    /* Copy back to P */
    matrixCopy(&P_new, &handle->P);
...
int8_t ekfQuatDerivative(float *q_dot, const float *q, const float *omega)
{
     ...
     * q_dot[0] = -0.5 * (q[1]*ωx + q[2]*ωy + q[3]*ωz)
     * q_dot[1] =  0.5 * (q[0]*ωx + q[2]*ωz - q[3]*ωy)
     * q_dot[2] =  0.5 * (q[0]*ωy - q[1]*ωz + q[3]*ωx)
     * q_dot[3] =  0.5 * (q[0]*ωz + q[1]*ωy - q[2]*ωx)
     */
                                 ...
                                 float dt, const EKF_Handle_t *handle)
{
    ...
    float omega[3];
    omega[0] = gyro[0] - x[4];  /* ωx - bx */
    omega[1] = gyro[1] - x[5];  /* ωy - by */
    omega[2] = gyro[2] - x[6];  /* ωz - bz */
                                  ...
                                  const EKF_Handle_t *handle)
{
     ...
     * a_meas = q* ⊗ [0, 0, -1] ⊗ q (in NED convention)
     * 
