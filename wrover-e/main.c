...
/* Initialize IMU with custom pins */
Ism330dhcxHandle_t imu;
Ism330dhcxPins_t pins = 
{
    .miso = GPIO_NUM_27,
    .mosi = GPIO_NUM_14,
    .sclk = GPIO_NUM_12,
...
static void taskImuRead(void)
{
    ...
    g_accel[1] = data.accel_g[1];
    g_accel[2] = data.accel_g[2];

    /* Store gyro (rad/s) */
    g_gyro[0] = CTRL_DEG_TO_RAD(data.gyro_dps[0]);
    g_gyro[1] = CTRL_DEG_TO_RAD(data.gyro_dps[1]);
    g_gyro[2] = CTRL_DEG_TO_RAD(data.gyro_dps[2]);

    // g_gyro[0] = data.gyro_dps[0];
    // g_gyro[1] = data.gyro_dps[1];
    // g_gyro[2] = data.gyro_dps[2];
...
static int8_t systemInit(void)
{
    ...
    /* Configure for flight controller */
    ret = ism330dhcxConfigureFlightController(&imu);
    if (ret != ISM330DHCX_OK) {
        ESP_LOGE(TAG, "IMU config failed: %d", ret);
        return -2;
    }
    ...
    /*=========================================================================
     * Rate Controller (Inner Loop) - 200 Hz
     *=========================================================================*/
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "╔══════════════════════════════════════════════╗");
    ESP_LOGI(TAG, "║  ESP32-WROVER-E Quadcopter Flight Controller ║");
    ESP_LOGI(TAG, "╚══════════════════════════════════════════════╝");
